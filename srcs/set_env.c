...
/*   Updated: 2018/10/07 13:35:23 by Awklm            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_strcmp(xml_parser("config/screen/set", &m->env.config_struct, 3, 0), "no") == 0)
	{
		m->env.resize_mode = 1;
	...
	{
		m->env.scr_width = ft_atoi(xml_parser("config/screen/width", &m->env.config_struct, 3, 0));
		m->env.scr_height = ft_atoi(xml_parser("config/screen/height", &m->env.config_struct, 3, 0));
	}
	return(0);
}
...
int		set_time(t_main *m, int option)
{
	...
	if (option == 0)
	{
		if (ft_atoi(xml_parser("config/time/first", &m->env.config_struct, 3, 0)) == 0)
		{
	...
	else if (option == 1)
	{
		...
		xml_parser(tmp, &m->env.config_struct, 4, 0);
		ft_strdel(&tmp);
		tmp_time = tmp_time + ft_atoi(xml_parser("config/time/total", &m->env.config_struct, 3, 0));
...
{
	if (ft_strcmp("english", xml_parser("config/lang/lang", &m->env.config_struct, 3, 0)) == 0)
		m->env.language_mode = 0;
	else if (ft_strcmp("french", xml_parser("config/lang/lang", &m->env.config_struct, 3, 0)) == 0)
		m->env.language_mode = 1;
	else
		m->env.language_mode = -1;
	if (set_struct_lang(m, m->env.language_mode) == -1)
		return(-1);
	return(0);
}
